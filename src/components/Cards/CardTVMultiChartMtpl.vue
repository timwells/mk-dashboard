<template>
	<div>
		<div :id="chartId" style="width: 99%; height: 500px;"></div>
	</div>
</template>
...
export default ({
	...
	watch: {
		chartCache(newVal, oldVal) {
			if(newVal !== null) {
				if(newVal.length === this.datasets.length) {
					for(let i = 0; i < this.datasets.length; i++) {
						switch(i) {
							...
							case 4: 
								...
								break;
							case 5: 
								this.setChartSeries(this.lineSeries5,newVal[5].datanewVal[5].name)
								break;
							case 6: 
	...
	mounted() {
		this.$store.dispatch("mtpl/resetChartDataValues");
		...
		for(let i = 0; i < this.datasets.length; i++) {
			...
			this.$store.dispatch("mtpl/getChartDataValues",{dataset: this.datasets[i]});
		}
  	},
  	beforeDestroy() {
    	if (this.chart) {
	...
	methods: {
		setChartSeries(lineSeries, series, name) {
			lineSeries.setData(series); 
			lineSeries.applyOptions({title: name})
		}
	}
})
