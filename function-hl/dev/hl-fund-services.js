...
async function queryFundsCount() {
    ...
    const urlFundsCount = "https://us-central1-mk-d-b59f2.cloudfunctions.net/hl/fundscount"

    const {data} = await axios.get(urlFundsCount,{headers: {'x-api-key': config.apiKeys[0]}})
    return data;
}
async function queryFunds() {
    ...
    while(start < totalFunds) {
        ...
        const resource = `https://us-central1-mk-d-b59f2.cloudfunctions.net/hl/fundscount/fundspage?start=${start}&rpp=${rpp}`
        console.log(resource)
        const { data } = await axios.get(resource,{headers: {'x-api-key': config.apiKeys[0]}})
        // console.log(data)
        if(data != null) {
