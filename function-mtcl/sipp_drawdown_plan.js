...
const calculateDrawdownPlanImpl = async () => {
  const plan = [];
  ...
  for (let year = modelVariables.startYear + 1, age = modelVariables.startAge + 1; age <= modelVariables.endAge; year++, age++) {
    ...
    plan.push({
      year,
      age,
      pensionPotStart: pensionPot.toFixed(1),
      withdrawal: withdrawal.toFixed(1),
      tax: tax.toFixed(1),
      gift: gift.toFixed(1),
      statePension: statePension.toFixed(1),
      taxableIncome: taxableIncome.toFixed(1),
      netIncomeAfterTaxAndGift: netIncome.toFixed(1),
      pensionPotGrowth: potGrowth.toFixed(1),
      pensionPotEnd: pensionPotEnd.toFixed(1)
    });
